...
class Solution:
    def findRadius(self, houses, heaters):
        ...
        while left < right:
            mid = (left + right) // 2
            if self.isValidRadius(houses, heaters, mid):
                right = mid
            else:
                left = mid + 1
        return left

    def isValidRadius(self, houses, heaters, radius):
        j = 0
        for i in range(len(houses)):
            while j < len(heaters) and abs(houses[i] - heaters[j]) > radius:
                j += 1
            if j == len(heaters):
                return False
        return True
...
sol = Solution()
print(sol.findRadius([1, 2, 3, 5, 15], [2, 30]))        
